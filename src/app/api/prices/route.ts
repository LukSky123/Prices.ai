...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (pricesError) {
      ...
      throw new Error(`Prices query failed: ${pricesError.message}`);
    }

    console.log(`📊 Found ${items?.length || 0} items and ${prices?.length || 0} prices`);
    ...
    console.log(`✅ Returning ${response.length} processed items`);
